...
class yunkai(object):
    ...
    def get_url_first(self):
        try:
            # 获取url列表
            sql = '''select url from bas_seller_all where shop_name="云开"'''
            ...
            for i in xrange(0,get_num):
                ...
                if http_ok:
                    html = resp.content
                    re_page = '''(?:<ul\sclass="p-page">.*?</span>/)(?P<page>\d+)(?:</a></li>)'''
    ...
    def do_main(self, url, page_num):
        ...
        try:
            for i in xrange(1, page_num + 1):
                # 遍历商品列表页
                get_url = re.sub(r'''.html''',"-0-sale-desc-0-0-min,max-" + str(i) + ".html", url)
                (http_ok, resp) = comm.requests_pkg.get(get_url, max_try=5)
                if http_ok:
                    html = resp.content
                    # 去噪
                    re_really = '''<div\sclass="goods-list\smt15">.*?<div\sclass="clear"></div>'''
                    re_detail_list = re.findall(re_really, html, re.S | re.I)
                    resp_body = re_detail_list[0]
                    # 获取商品项
                    re_detail = '''(?:<div\sclass="pd10\slist-top">)(?P<detail>.*?)(?:<div\sclass="list-bottom">)'''
                    ...
                    for deatil_item in detail_list:
                        ...
                        if get_url:
                            ...
                            item["prod_url"] = "http://www.ykyao.com" + product_uri
                        # 获取产品ID
                        item["prod_id"] = ""
                        re_detail_id = '''-(\d+)'''
                        ...
                        # 产品查重
                        sql = '''select id from `wrk_seller_all` where product_id="%s" and shop_id="100007"''' % product_id
                        (get_num,get_item) = self.db_oper.exe_search(sql)
                        if get_num > 0:
                            comm.PLog.Log("商品重复")
                            continue
                        ...
                        re_price = '''(?:<span\sclass="red.*?¥)(?P<price>\d+|\d+.\d+)(?:</span>)'''
