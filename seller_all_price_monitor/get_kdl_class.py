        ...
        try:
            for i in xrange(1, page_num + 1):
                ...
                if http_ok:
                    ...
                    # 获取商品项
                    re_detail = '''(?:<li\sclass=)(?P<detail>.*?)(?:</li>)'''
                    ...
                    for deatil_item in detail_list:
                        # 获取url
                        item["prod_url"] = ""
                        re_url = '''(?:<div\sclass="pic">\s*<a\shref=")(?P<url>goods-\d+.html)(?:"\s)'''
                        get_url = re.search(re_url, deatil_item, re.S | re.I)
                        if get_url:
                            product_uri = get_url.group("url")
                            url_real = "http://www.baiji.com.cn/%s" % str(product_uri)
                        ...
                        # 产品查重
                        sql = '''select id from `wrk_seller_all` where product_id="%s" and shop_id="100003"''' % product_id
                        (get_num,get_item) = self.db_oper.exe_search(sql)
                        if get_num > 0:
                            comm.PLog.Log("商品重复")
                            continue
                        ...
                        re_price = '''(?:<strong>￥)(?P<price>\d+|\d+.\d+)(?:</strong>)'''
