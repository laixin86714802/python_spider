...
class yunkai(object):
    ...
    def get_url_first(self):
        try:
            # 获取url列表
            sql = '''select url from bas_seller_all where shop_name="康之家"'''
            ...
            for i in xrange(0,get_num):
                ...
                if http_ok:
                    html = resp.content
                    re_page = '''(?:<div\sclass="pagin.*?</i>/)(?P<page>\d+)(?:</span>)'''
    ...
    def do_main(self, url, page_num):
        ...
        try:
            for i in xrange(1, page_num + 1):
                # 遍历商品列表页
                get_url = re.sub(r'''.html''',"-min0-max0-page-" + str(i) + "-sales_base_number-DESC-pre2.html", url)
                (http_ok, resp) = comm.requests_pkg.get(get_url, max_try=5)
                if http_ok:
                    html = resp.content
                    # 去噪
                    re_really = '''<ul\sclass="goodsItem\sclearfix">.*?<div\sclass="selectPageForm">'''
                    re_detail_list = re.findall(re_really, html, re.S | re.I)
                    resp_body = re_detail_list[0]
                    # 获取商品项
                    re_detail = '''(?:<li>)(?P<detail>.*?)(?:</a></p>)'''
                    ...
                    for deatil_item in detail_list:
                        ...
                        if get_url:
                            ...
                            item["prod_url"] = "http://www.kzj365.com" + product_uri
                        # 获取产品ID
                        item["prod_id"] = ""
                        re_detail_id = '''-(\d+)'''
                        ...
                        # 产品查重
                        sql = '''select id from `wrk_seller_all` where product_id="%s" and shop_id="100008"''' % product_id
                        (get_num,get_item) = self.db_oper.exe_search(sql)
                        if get_num > 0:
                            comm.PLog.Log("商品重复")
                            continue
                        ...
                        re_price = '''(?:<div\sclass="price">￥)(?P<price>\d+|\d+.\d+)(?:<span)'''
