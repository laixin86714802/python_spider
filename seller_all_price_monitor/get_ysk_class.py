...
class yishengkang(object):
    ...
    def get_url_first(self):
        try:
            # 获取url列表
            sql = '''select url from bas_seller_all where shop_name="亿生康"'''
            ...
            for i in xrange(0,get_num):
                ...
                if http_ok:
                    html = resp.content
                    re_page = '''(?:<div\sclass="page_b">.*?</strong>/)(?P<page>\d+)(?:</div>)'''
    ...
    def do_main(self, url, page_num):
        ...
        try:
            for i in xrange(1, page_num + 1):
                # 遍历商品列表页
                get_url = re.sub(r'''.html''',"-min0-max0-attr0-" + str(i) + "-goods_id-DESC.html", url)
                (http_ok, resp) = comm.requests_pkg.get(get_url, max_try=5)
                if http_ok:
                    html = resp.content
                    # 去噪
                    re_really = '''<div\sclass="prorig_product_list".*?<div\sclass="prorig_filter">'''
                    re_detail_list = re.findall(re_really, html, re.S | re.I)
                    resp_body = re_detail_list[0]
                    # 获取商品项
                    re_detail = '''(?:<li\sclass="prorig_product">)(?P<detail>.*?)(?:</li>)'''
                    ...
                    for deatil_item in detail_list:
                        ...
                        # 产品查重
                        sql = '''select id from `wrk_seller_all` where product_id="%s" and shop_id="100006"''' % product_id
                        (get_num,get_item) = self.db_oper.exe_search(sql)
                        if get_num > 0:
                            comm.PLog.Log("商品重复")
                            continue
                        ...
                        # 产品名称
                        item["prod_name"] = ""
                        re_prod_name = '''(?:<a\shref=".*?alt=")(?P<name>.*?)(?:">\s</a></div>)'''
                        ...
                        re_price = '''(?:<strong>￥)(?P<price>\d+|\d+.\d+)(?:元</strong>)'''
                        get_price = re.search(re_price, deatil_item, re.S | re.I)
                        if get_price:
                            item["prod_price"] = get_price.group("price")
                        # 获取规格
                        item["spec"] = ""
                        re_spec = '''(?:<p\sclass="speci">\s规格：)(?P<spec>.*?)(?:\s</p>)'''
                        get_spec = re.search(re_spec, deatil_item, re.S | re.I)
                        if get_spec:
                            item["spec"] = get_spec.group("spec")
                        # 获取产地
                        item["prod_place"] = ""
                        re_prod_place = '''(?:<p\sclass="speci">\s厂家：)(?P<prod_place>.*?)(?:\s</p>)'''
                        get_prod_place = re.search(re_prod_place, deatil_item, re.S | re.I)
                        if get_prod_place:
