...
class yiyaowang(object):
    ...
    def get_url_first(self):
        try:
            # 获取url列表
            sql = '''select url from bas_comweb_url where comp_id="100001"'''
            ...
            for i in xrange(1, page_num + 2):
                ...
                if http_ok:
                    ...
                    for deatil_item in detail_list:
                        ...
                        # 产品查重
                        sql = '''select id from `wrk_yyw_price` where product_id="%s" and shop_id="100001"''' % product_id
                        (get_num,get_item) = self.db_oper.exe_search(sql)
                        if get_num > 0:
                            comm.PLog.Log("商品重复")
                            continue
    ...
    def load_to_db(self, item):
        try:
            ...
            sql = "insert into wrk_yyw_price(shop_id, product_id, product_name, approval_num, spec, product_place, product_price, product_url, collect_time) values(%s, %s, %s, %s, %s, %s, %s, %s, %s)"
            vals = (
                item["shop_id"],
                item["prod_id"],
                item["prod_name"],
