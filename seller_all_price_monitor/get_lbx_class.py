...
class laobaixing(object):
    ...
    def get_url_first(self):
        try:
            # 获取url列表
            sql = '''select url from bas_seller_all where shop_name="老百姓"'''
            ...
            for i in xrange(0,get_num):
                ...
                if http_ok:
                    html = resp.content
                    re_page = '''(?:<span\s*class="font-red">\d+</span>/<span>)(?P<page>\d+)(?:</span>)'''
    ...
    def do_main(self, url, page_num):
        item = {}
        try:
            url = url + "?q=&text=&page=0"
            for i in xrange(1, page_num + 1):
                # 遍历商品列表页
                get_url = re.sub(r'''page=\d+''',"page=" + str(i-1) + "", url)
                (http_ok, resp) = comm.requests_pkg.get(get_url, max_try=5)
                if http_ok:
                    html = resp.content
                    # 去噪
                    re_really = '''<div\sclass="search_cholist\smtp20">.*?<script\stype="text/javascript'''
                    re_detail_list = re.findall(re_really, html, re.S | re.I)
                    resp_body = re_detail_list[0]
                    # 获取商品项
                    re_detail = '''(?:<div\sclass="ucol_p_img">)(?P<detail>.*?)(?:<div\sclass="ucol_p_icon">)'''
                    ...
                    for deatil_item in detail_list:
                        ...
                        # 产品查重
                        sql = '''select id from `wrk_seller_all` where product_id="%s" and shop_id="100005"''' % product_id
                        (get_num,get_item) = self.db_oper.exe_search(sql)
                        if get_num > 0:
                            comm.PLog.Log("商品重复")
                            continue
                        ...
                        re_price = '''(?:<span\sclass="font-red.*?">\s*￥)(?P<price>\d+|\d+.\d+)(?:</span>)'''
