...
class ip_area_server_class():
    ...
    def ip_area_request(self, ip):
        ...
        if http_ok:
            ...
            if len(addr_list) > 5:
                ...
                #--------------------------------------------------------------
                # 作  者： wltsoft
                # 时  间： 2016-05-30 10:02
                # 备注头： 这种数据若不记录到到redis，则会导致大量重复的无效请求
                # 备注体：
                #--------------------------------------------------------------
                comm.PLog.Log("请求淘宝接口")
                return_info = self.ip_area_request_taobao(ip)
                comm.PLog.Log("==>" + return_info)
                self.save(ip, prov_name, "")
            else:
    ...
    def ip_area_request_taobao(self, ip):
        ...
        if http_ok:
            ...
            all_info = u" ".join(result)
            comm.PLog.Log(all_info)

            contry_name = json_data[u'data'][u'country'].encode('utf-8')

            fmt_prov_name = self.fmt_area_info(
                self.m_province_arr, all_info)
            fmt_city_name = self.fmt_area_info(self.m_city_arr, all_info)
            comm.PLog.Log("fmt_prov_name=%s" % fmt_prov_name)
            comm.PLog.Log("fmt_city_name=%s" % fmt_city_name)

            # 优先取适配数据，取不到则取原数据
            if fmt_prov_name == '':
                #取国家名
                fmt_prov_name = contry_name
                fmt_city_name = ""

            self.save(ip, fmt_prov_name, fmt_city_name)
            return_info = '%s:%s' % (fmt_prov_name, fmt_city_name)
        else:
            comm.PLog.Log("ip %s 请求失败,放弃!" % ip)
        ...
        try:
            ...
            if val:
                for item in arr:
                    ...
                    #comm.PLog.Log("--->%s:%s" % (item, val))
                    if re.search(item, val, re.S | re.I ):
                        ret_fmt_info = item
                        break
        except:
            pass
