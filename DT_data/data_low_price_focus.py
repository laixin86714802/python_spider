...
class low_price_focus(object):
    ...
    def get_low_price_focus(self):
        '''获取低价集中数据'''
        sql_low_price_focus = '''SELECT A.date_time, M.count, count(*) as 'all_count', M.count / count(*) as 'low_price_per'
            FROM tb_mk_dt_lowprice_focus_day_tmp as A,
            (SELECT date_time, count(*) as 'count' FROM tb_mk_dt_lowprice_focus_day_tmp
        ...
        for low_price_focus in res_low_price_focus:
            ...
            data_item['count'] = low_price_focus['count']
            data_item['all_count'] = low_price_focus['all_count']
            data_item['low_price_per'] = low_price_focus['low_price_per']
            self.load_to_db(data_item)

    def load_to_db(self, data_item):
        '''入库函数'''
        collecttime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(time.time())))
        sql = '''INSERT INTO tb_shw_dt_lowprice_focus_day(date_time, jk_low_price_num, all_count, low_price_per, collect_time) 
                VALUES(%s, %s, %s, %s, %s)'''
        vals = (
            data_item['date_time'],
            data_item['count'],
            data_item['all_count'],
            data_item['low_price_per'],
            collecttime
            )
