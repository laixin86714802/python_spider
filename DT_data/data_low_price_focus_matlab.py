...
class low_price_focus_pretreatment(object):
    ...
    def get_comp_mapping(self, jk_prod_id, date):
        ...
        for jk_prod_mapping in res_jk_prod_mapping:
            ...
            comp_price_item[data_item['comp_id']] = comp_price
    ...
    def load_to_db(self,data_item):
        ...
        sql = '''INSERT INTO tb_mk_dt_lowprice_focus_day_matlab(jk_id, date_time, jk_price, comp_id, comp_price_min, jk_low_price, collect_time) 
    ...
    def load_to_db_tmp(self,data_item):
        '''入库函数'''
        collecttime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(time.time())))
        sql = '''INSERT INTO tb_mk_dt_lowprice_focus_day_matlab(jk_id, date_time, jk_price, comp_id, comp_price_min, jk_low_price, collect_time) 
                VALUES(%s, %s, %s, %s, %s, %s, %s)'''
...
data_item = {}
date_item = a.datelist((2016, 11, 23), (2016, 11, 27))
num = 0
for date in date_item:
    for jk_prod_id in jk_prod_id_item:
        ...
        comp_price_item = a.get_comp_mapping(jk_prod_id=jk_prod_id, date=date)
        comp_id = [i[0] for i in comp_price_item.items()]
        comp_price = [i[1] for i in comp_price_item.items()]
        data_item['comp_id'] = comp_id[comp_price.index(min(comp_price))]
        ...
        if data_item['jk_price'] and data_item['comp_price_min']:
            if data_item['jk_price'] <= data_item['comp_price_min']:
                data_item['comp_id'] = 100000
                data_item['jk_low_price'] = 1
            else:
                data_item['jk_low_price'] = 0
            num += 1
            a.load_to_db_tmp(data_item)
            if num % 200 == 0:
                a.db.commit()